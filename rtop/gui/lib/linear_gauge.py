...
def basic_gauge(stdscr, pos_y, pos_x, size_w, data, bar='|'):
    """Draw a polished linear gauge bar matching jtop style.

    data = {
        'name': short name label
        'color': color for the name
        'online': bool status
        'message': Offline message (default "OFF")
        'coffline': Color offline (default RED)
        'values': [(value, color), ...] sum of values = 100
    ...
    stdscr.addstr(pos_y, pos_x + name_size, "[" + " " * size_bar + "]",
                  curses.A_BOLD if online else color_offline)
    # Draw bar
    if online:
        ...
        # Draw all values with colors
        x_bar_start = 0
        old_val = 0
        for value, color in values:
            x_bar_end = int((old_val + value) * size_bar) // 100
            stdscr.addstr(pos_y, pos_x + name_size + x_bar_start + 1,
                          str_progress_bar[x_bar_start:x_bar_end], color)
        ...
        # Show message status
        try:
            stdscr.addstr(pos_y, pos_x + name_size + 2,
                          data['message'] if 'message' in data else "OFF", color_offline)
        except curses.error:
            pass


def basic_gauge_simple(stdscr, pos_y, pos_x, size, freq_data, unit='k'):
    """Draw a simple frequency gauge with running/off indicator."""
...
def freq_gauge(stdscr, pos_y, pos_x, size, freq_data):
    """Draw a frequency gauge with min/max range, matching jtop style."""
    ...
    # Render current frequency string
    ...
    data = {
        'name': name,
        'color': NColors.cyan(),
        'online': freq_data.get('online', True),
